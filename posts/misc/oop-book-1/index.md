...
우리는 우리 눈에 보이는 것들을 목적에 따라 추상화하여 바라본다. 여기서 추상화란 내게 필요한 사실만 추려서 보고 나머지는 무시하는 것이다.

내가 휴가를 나온 군인 친구를 만난다고 하자. 그 친구는 군대에서 자신의 직급과 역할을 가지고 있을 것이다. 하지만 그것이 무엇이든 내게는 그리 중요하지 않다. 중요한 건 그 친구가 군인인 것이 아니라 내 친구라는 사실이기 때문이다.

반면 내가 경계를 서고 있는 군인들을 본다고 하자. 그러면 나는 그 군인들을 그저 군인이라는 개념으로 바라볼 것이다. 그들 각자의 이름이나 인간관계는 무시한 채 단순하게 말이다. 철수 대신 민수가 그 자리에 군복과 총을 들고 서 있어도 내게는 아무런 차이도 없다. 그들 각자는 내가 헤아릴 수도 없을 만큼 많은 정보를 지니고 있겠지만 나는 군인이라는 공통점만 보고 있다.

추상화는 결국 사물에서 내게 필요한 본질만 뽑아내어 볼 수 있도록 하는 과정이다. 이는 사물들 간의 공통점만 취해서 일반화하는 차원, 그리고 중요한 부분 외에 불필요한 세부 사항을 제거하는 차원의 2가지 차원으로 이뤄진다.

## 4.2. 타입

프로그래밍에서는 이렇게 추상화한 사물들을 타입이라는 개념으로 표현한다. 만약 어떤 객체가 A 타입에 속한다면, 해당 객체는 A타입의 인스턴스라고 생각하는 식이다.

타입은 객체를 분류하는 데 사용되며, 객체의 분류는 객체가 수행하는 행동에 기반한다. 즉, 객체가 수행하는 행동이 같다면 같은 타입에 속한다고 말할 수 있다.
