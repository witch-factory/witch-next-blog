...
이 라이브러리의 `visit(tree[, test], visitor[, reverse])`을 사용하면 트리의 각 노드에 대해 visitor 함수를 실행할 수 있다. 이걸 리턴되는 함수에 넣어주자.
...
이 라이브러리의 `visit`을 이용하면 preorder의 depth-first-search를 해준다. 그러면서 HTML AST의 각 노드를 편집할 수 있다.

또한 remark 플러그인 함수는 tree, file을 인수로 받는 함수를 리턴하는데, tree는 mdast이고 file은 contentlayer에서 제공하는 파일 정보를 담고 있는 객체다. 이 file인수를 통해서 우리는 파일의 경로를 알 수 있다.
