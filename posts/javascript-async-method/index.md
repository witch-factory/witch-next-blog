...
이런 비동기를 처리하는 전통적인 방식으로 이벤트 핸들러를 생각할 수 있다. 비동기 작업을 완료하는 이벤트에 대해 핸들러를 등록하고 해당 이벤트가 발생하면 어떤 콜백을 실행하도록 하는 것이다.

초기의 비동기 API는 이런 방식을 사용하고는 했다. `fetch` API 이전에 HTTP 요청을 보낼 때 쓰이던 XMLHttpRequest가 이를 사용하던 대표적인 객체였다. XHR 객체의 `loadend`이벤트가 발생하면 특정 콜백을 실행하도록 한 것이다.
...
하지만 이렇게 콜백을 이용할 경우, 비동기 작업이 순서대로 일어나는 것을 처리하기 어렵다. 이전에 진행되던 비동기 작업의 맥락을 유지해 줘야 하기 때문이다.
