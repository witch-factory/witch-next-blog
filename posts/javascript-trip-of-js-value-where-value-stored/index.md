...
이 시리즈는 "Javascript의 값은 어디서 왔고 무엇이며 어디로 가는가" 시리즈 중 첫번째 글이다.

![우리는 어디서 왔고 무엇이며 어디로 가는가](./Paul_Gauguin_Dou_venons_nous.jpg)

폴 고갱, 1897, "우리는 어디서 왔고, 우리는 무엇이며, 우리는 어디로 가는가"

| 제목 | 링크 |
|------|------|
| JS의 값은 스택과 힙 중 어디에 저장되는가? | 링크 |

이 글은 [파차님의 트윗](https://twitter.com/finalchildmc/status/1751818395669106722)을 보고 영감을 얻어 작성하게 되었다.

이 글은 Javascript의 값이 어디에 저장되는지에 대해 다룬다. Javascript의 값이 왜 힙에 저장되어야 하며 명세에서 그리고 엔진들에서 어떻게 다루는지를 간략히 이야기하는 글이다.

# 1. 흔한 설명

Javascript의 값은 원시값과 객체로 나뉜다. 원시값은 숫자, 문자열, 불리언, null, undefined, 심볼이 있고 객체는 그 외의 함수, 배열, Map 등 모든 것이다. 그럼 이러한 값들은 어디에 저장될까?
...
그러나 이런 설명은 틀렸다. Javascript에서는 원시값을 포함한 모든 것이 원칙적으로 힙에 저장되고 그걸 가리키는 포인터를 통해 사용된다. 물론 엔진마다 약간씩 다른 최적화 기법들이 있고 여기에 따라 몇몇 값들이 스택에 직접 저장되기도 한다. 글의 뒷부분에서 이에 대해 다루기도 할 것이다.

하지만 Javascript의 모든 값은 원칙적으로 힙에 저장된다.

# 2. 흔한 설명의 문제

객체는 유명한 설명대로 힙에 저장된다고 치고, 원시값이 스택에 그대로 저장된다고 하자. 이는 대표적으로 2가지 문제가 있다.
...
만약 변수에 늘 같은 타입의 원시값만 저장된다고 해도 원시값에 문자열도 포함된다는 게 문제다. 문자열은 길이에 따라 차지하는 메모리 크기가 달라지기 때문에 얼만큼의 메모리를 할당해야 하는지 알 수가 없다.

## 2.2. 클로저의 문제

물론 이는 문자열의 경우 참조(혹은 포인터라고 해도 된다)를 통해 다루어지며 이 참조를 스택에 저장하면 된다고 할 수도 있다. 하지만 그렇게 한다 해도 클로저 때문에 발생하는 문제가 있다.
...
가령 다음과 같은 흔한 클로저 예시 코드에서 `count`는 원시값이므로 스택에 저장된다고 하자. `count`는 숫자이므로 메모리 할당에 대한 문제는 없다. 하지만 `count`는 내부 함수의 클로저를 통해 접근 가능하므로 함수가 종료되어도 `count`는 메모리에서 사라지지 않아야 한다.
...
하지만 Javascript는 모든 값을 힙에 저장하고, 따라서 이런 문제가 없다.

# 3. 명세에서

그럼 ECMA-262 명세에서는 값의 저장에 대해서 어떻게 정의하고 있을까? 모든 Javascript 구현체는 ECMA-262 명세를 따르기 때문에 이를 통해서 값이 어떻게 저장되어 있는지 엿볼 수도 있을 것이다.
...
사실 대부분의 언어 명세에서 무엇이 스택에 저장되고 힙에 저장되는지는 커녕 스택, 힙이라는 언급조차 하지 않는 것을 생각해 볼 때 이렇게 값이 간접적으로 참조된다고 명시하는 것은 값을 힙에 저장한다는 것을 꽤나 명확하게 암시하는 것이라고 볼 수 있다.
...
먼저 '변수'와 '변수가 가리키는 것' 즉 실제 값을 구별해야 한다. 변수는 일종의 포인터로 볼 수 있고 다른 어딘가에 저장된 객체를 가리키는 참조를 가지고 있다. 이는 모든 변수가 포인터 하나만큼의 크기만을 차지하며 변수가 시간에 따라 다른 객체를 참조하게 되는 경우에도 변수가 필요로 하는 메모리 크기가 변하지 않는다는 것을 의미한다.
...
반면 변수가 가리키는 값이란 변수에 저장된 포인터가 가리키는 값 그 자체를 가리킨다. 당연히 크기가 고정되어 있지 않다.

## 4.2. 저장

앞서 언급했듯이 Javascript에서 모든 값들은 원칙적으로 힙에 저장된다. 따라서 모든 변수(에 담긴 포인터)와 변수가 가리키는 값들 모두가 힙에 저장되어도 문제는 없다.

다만 최신 Javascript 엔진들은 대부분 함수의 지역 변수들을 스택에 저장한다. **변수가 가리키는 값을 저장하는 게 아니고 변수를 저장하는 것이다.** 변수가 무엇을 가리키건 포인터 하나만큼의 메모리씩만 차지하고 있기에 메모리 할당에 대한 문제도 없고 속도도 빠르기 때문이다.

그런데 만약 클로저, `eval`의 사용 등의 문제로 함수가 종료된 이후에도 변수가 메모리에 남아 있어야 한다면 이러한 값은 스택에 저장할 수 없다. 따라서 엔진은 어떤 변수가 함수 종료 이후에도 메모리에 존재해야 하는지 추적하고 이 변수들은 힙에 저장한다. 보통 클로저에 사용되는 변수들이 이에 해당한다.

변수가 가리키는 값도 원시값이든 객체든 원칙적으로 모두 힙에 저장되지만 V8이 31비트 범위의 부호 있는 정수를 스택에 저장하고 SpiderMonkey가 NaN-boxing이라는 테크닉을 사용하는 등 예외도 있다. 글의 뒷부분에서 이에 대해서도 다룰 예정이다. 하지만 이는 일종의 최적화로서 엔진마다 다르고, 원칙적으로는 모든 값들은 힙에 저장된다.

이외에도 함수의 호출이 많아져서 특정 부분의 코드가 최적화가 요구될 만큼 'hot'해지면 객체의 특정 프로퍼티를 스택에 저장하는 등 엔진에서 다른 최적화를 하기도 한다. 최적화를 다룰 이후 글에서 다룰지도 모른다. 하지만 이 또한 엔진마다 다른 일종의 최적화이다.

# 5. 엔진에서 - SpiderMonkey

실제 Javascript 엔진에서는 값이 어떻게 저장되는지 알아보자. 먼저 주요 엔진 중 하나인 SpiderMonkey에서는 어떻게 값이 저장되는지 알아보자. [SpiderMonkey 공식 사이트](https://spidermonkey.dev/)에서는 [SpiderMonkey의 코드를 찾아볼 수 있는 페이지](https://searchfox.org/mozilla-central/source/js/src)가 있다. 이곳의 검색 기능을 활용하면 이 글과 같은 코드를 찾아볼 수 있다.
