...
2. 자바스크립트가 만들어질 때 영향을 받은 언어들
3. 자바스크립트의 초기 설계시 고려된 것들
...
1989년에서 1991년 사이 팀 버너스-리가 월드 와이드 웹과 HTML을 개발했다. 하지만 당시에는 별로 큰 주목을 받지 못했다. 이것이 주목받기 시작한 건 1993년 1월 마크 앤드리슨과 에릭 비나가 만든 웹브라우저 모자이크가 출시되면서였다.

이 브라우저는 기술적인 지식이 없는 사람들도 쉽게 사용할 수 있었고 윈도우즈를 지원했다. 그리고 사진 등의 멀티미디어를 페이지 내에서 볼 수 있었다. 기존 브라우저에서는 이미지 등의 멀티미디어를 보기 위해서 링크를 클릭하여 새 창에 이미지를 띄워야만 했다. 이런 여러 장점을 갖췄던 모자이크는 초기 웹이 널리 퍼져나가는 데에 큰 역할을 했다.

이 모자이크를 개발한 리드 개발자들은 인터넷의 역사에서 꽤나 자주 등장하는 넷스케이프를 설립하고 1994년 12월 더 세련된 브라우저인 넷스케이프 네비게이터 1.0을 출시했다. 이는 빠르게 모자이크를 대체하고 주류가 되었다.

## 1.2. 브라우저에 멀티미디어 넣기

HTML은 웹 브라우저에서 인터넷을 통해 문서를 볼 수 있게 해주었다. 이는 엄청난 혁신이었다. 그러나 넷스케이프는 영리 기업이었기에 뭔가 초기의 웹에 가치를 더하여 돈을 벌 방법을 찾아야 했다. 그렇게 하기 위해서는 어쩌면 당연하게도 웹에서 멀티미디어를 잘 볼 수 있도록 해야 했다.

이는 1993년 이미 마크 앤드리슨이 모자이크를 만들 때부터 고려되었던 것이었다. 그는 그때부터 이미지를 페이지에 삽입하여 보여주는 `<img>` 태그를 HTML 표준에 넣어야 한다고 제안했다. 그리고 1993년 마크 앤드리슨은 HTML 표준에 `<img>`가 들어가지 않았음에도 불구하고 모자이크에 해당 기능을 포함시켰다. 모자이크는 그렇게 멀티미디어를 인라인으로 삽입할 수 있다는 장점을 갖추고 널리 퍼졌다.

하지만 당시에는 웹의 속도가 끔찍하게 느렸다. 따라서 넷스케이프 1.0은 이런 환경 하에서 브라우저가 기본적인 멀티미디어를 더 잘 다룰 수 있게 하는 데에 집중해야 했다. 사용자가 보려는 이미지를 로딩하는 동안 텍스트를 먼저 보여주는 최적화 등의 작업이었다.

## 1.3. 동적인 웹을 향해서

1995년 즈음에는 웹에서 멀티미디어를 다루는 게 어느 정도 안정되었다. 그래서 이제 넷스케이프는 웹에서 멀티미디어를 다루는 걸 넘어 웹 페이지와 사용자의 상호작용이 가능하도록 만들고자 했다. 넷스케이프의 창립자 마크 앤드리슨 또한 웹이 더 동적으로 동작해야 한다고 생각했다.

이 시기의 웹페이지들은 페이지가 로드되고 나면 서버와의 상호작용 없이는 동적으로 뭔가를 할 수 없었다. 당시에는 사용자가 폼에 입력한 값을 검증할 때조차도 서버에 데이터를 보내고 피드백을 받아야 했다!

당시의 HTML만으로는 페이지가 동적으로 움직이고 사용자와 제대로 상호작용하는 게 불가능했다. 웹 페이지에 동적인 무언가를 넣을 수 있게 해줄 언어가 필요했다. HTML에 프로그래밍 언어를 삽입할 수 있도록 해야 했다. 이제야 새로운 언어 이야기가 나온다.
...
그리고 1995년 4월 브랜든 아이크를 고용했다. 그가 Silicon Graphics Inc.와 MicroUnity라는 회사에서 "커널과 네트워크 작업을 할 수 있는 작은 특수목적 언어"를 만들어 본 적이 있기 때문이었다.

하지만 아이크가 처음부터 새로운 언어를 만들고자 한 건 아니었다. 그는 기존 언어를 브라우저에 도입하고자 했고 Scheme을 브라우저에 구현하겠다고 생각하고 있었다. 넷스케이프에서도 Scheme을 구현하게 해주겠다고 했다고 한다.("come and do Scheme in the browser")
...
넷스케이프는 1994년 말 마이크로소프트의 저가 인수 제안을 거절했다. 그 이후 넷스케이프 경영진은 넷스케이프가 마이크로소프트의 "Embrace, extend, and extinguish"(마이크로소프트가 경쟁사를 공격하기 위해 수립한 내부 전략이었다) 전략에 의해 직접적으로 공격받을 것으로 예상했다. 따라서 넷스케이프는 웹 시장을 선점하기 위해 서두르려고 했다.
...
여담이지만 지금 상황을 보면 알 수 있듯이 이는 자바스크립트만 빼고 모두 실패하였다. 넷스케이프는 IE에 졌고 Java Applet은 실패했다. 자바스크립트만 남아서 지금의 웹을 이끌고 있다.
...
첫번째 의문은 쉽게 반박되었다. Java는 물론 지금도 주류를 차지하고 있을 만큼 좋은 언어다. 하지만 당시의 초심자들이나 전문 프로그래머가 아닌 사람들에게는 부적합했다. 범용 목적 언어인 Java는 웹 디자이너와 다른 웹의 구성원들이 바로 사용하기에는 너무 복잡했던 것이다.
...
그래서 아이크는 본인이 생각하는 중요한 사항을 빼면 나머지는 매우 유연하게 만들었다. 이 중요한 사항 2가지는 Scheme에서 온 일급 객체 함수의 개념과 Self의 프로토타입 기반의 객체지향 시스템이었다. 이렇게 자바스크립트가 영향을 받은 언어들과 중요 사항에 대해서는 다음 섹션들에서 자세히 다룬다.

아무튼 아이크는 이렇게 촉박하게 언어를 제작하면서도 자바스크립트가 여러 프로그래밍 패러다임에 맞춰질 수 있는 유연한 언어가 되도록 했다.

아이크는 자신이 자바스크립트를 이렇게 유연하게 만들어 놓으면 내장 라이브러리 등은 날림으로 해두어도 다른 개발자들이 수정하리라고 생각했다. 실제로 자바스크립트의 내장 라이브러리는 초기에 날림으로 만들어진 게 많다.

이런 이유로 자바스크립트는 빌트인 객체를 포함해서 모든 객체들의 수정을 거의 허용한다. `Object.prototype`과 같은 객체들을 수정할 수 있는 것도 이런 설계 때문이다. 이는 폴리필을 만들 수 있는 기반이 되기도 한다.
...
정규식과 문자열 매칭 관련 문법도 Perl에서 가져왔다.
...
프로토타입은 자바스크립트에서 객체지향을 구현하는 방식이다. 그리고 브랜든 아이크의 여러 인터뷰와 발표를 보면 일급 객체 함수와 함께 프로토타입이 '자바스크립트에서 잘 만들어진 부분'으로 자주 언급된다. 물론 급하게 만들어진 부분에서의 실수는 있었다고 인정하지만 말이다.

하지만 클래스 언어를 사용해본 사람들에게 이는 매우 낯선 부분이다. 그래서 이후 자바스크립트에 클래스가 추가되기도 했지만, 프로토타입은 분명 그 당시로서는 자바스크립트에 들어갈 만한 기능이었다. 프로그래밍 초보자를 위한 언어였던 자바스크립트에서 클래스는 잘 쓰기에 너무 어려웠기 때문이다.

[자바스크립트는 왜 프로토타입을 선택했을까](https://medium.com/@limsungmook/%EC%9E%90%EB%B0%94%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8%EB%8A%94-%EC%99%9C-%ED%94%84%EB%A1%9C%ED%86%A0%ED%83%80%EC%9E%85%EC%9D%84-%EC%84%A0%ED%83%9D%ED%96%88%EC%9D%84%EA%B9%8C-997f985adb42)라는 아주 유명한 글이 있다. 다 맞는 이야기는 아니지만, 철학적 배경에 대해서는 이 글을 참고하자. 이 글에서는 좀더 실질적인 이야기를 해보려고 한다.
...
예를 들어서 동물들을 클래스로 모델링한다고 해보자. 확장 가능하고 동물의 본질적인 것들을 갖춘 클래스는 어떻게 짤 것인가? 예를 들어서 다음과 같이 짜볼 수 있다. Java 코드이다.
...
그리고 이를 상속하는 `Cat`과 `Dog` 클래스도 만들 수 있다.
...
하지만 만약 `Fish`클래스도 만들어야 한다면? 다음과 같이 만들면 어떨까?
...
이 코드에는 문제가 있다. 물고기 클래스도 `Animal`을 상속하고 있기 때문에 `walk`메서드를 가지게 된다는 것이다. 물고기는 걷지 못하는데도 말이다. 그런데 물고기도 동물인 것은 맞으니 `Animal`을 상속하도록 하는 게 자연스럽다. `Animal` 클래스에 `walk` 메서드를 만든 것부터 잘못된 것이다.

물론 이런 문제를 해결하는 법은 여러 가지 있다. `Animal`을 수정하고 `Cat`과 `Dog`의 상위 클래스가 되는 `LandAnimal`과 같은 클래스를 만들 수도 있고 `Cat`, `Dog` 각각에 `walk` 메서드를 만들 수도 있다. 다 괜찮다.

하지만 문제는 결국 '클래스를 잘 모델링할 수 있어야 한다'는 것이다. 어떤 객체의 본질이라는 걸 찾아서 클래스를 모델링하기가 절대 쉽지 않음에도 말이다. 프로토타입을 사용하면 이런 문제를 해결할 수 있다.
...
다음은 위에서 `Fish` 객체를 만드는 방식을 JS의 프로토타입을 이용해서 모델링해본 것이다.
...
그리고 Java가 이미 클래스 기반이었기 때문에 자바스크립트도 클래스 기반의 객체지향을 사용하면 Java와 경쟁하게 되었을 것이다. 그래서 프로토타입 기반의 객체지향을 사용하면 Java와 경쟁하지 않고도 Java의 보완재 역할을 할 수 있었다.

다른 이유로는 계속 언급되었듯이 아이크에게는 시간이 얼마 없었는데, 프로토타입은 클래스보다 구현하기가 쉬웠다. [또한 수많은 내장 라이브러리와 빌트인 메서드들을 사용자가 직접 prototype을 통해 수정할 수 있게 함으로써 자바스크립트 언어를 최소화할 수도 있었다.](https://brendaneich.com/2005/06/자바스크립트-1-2-and-in-between/)
...
Self에서는 각 객체가 그 프로토타입 객체를 참조하는 링크를 가지고 있어서 같은 프로토타입 객체를 가지는 객체들은 공통적으로 그 프로토타입 객체가 제공하는 기능을 사용할 수 있었다. 이걸 약간 변형한 것이 자바스크립트의 프로토타입 모델이라고 할 수 있다.

프로토타입 객체는 생성자 함수의 `prototype` 프로퍼티를 통해서 간접적으로 접근되고 `new` 연산자는 생성자 함수의 `prototype` 을 프로토타입 객체로 갖는 새로운 인스턴스를 생성한다. 인스턴스는 생성자 함수의 `prototype`의 속성들을 사용할 수 있다. 이는 Self의 프로토타입 객체를 참조하는 링크와 비슷하다.
