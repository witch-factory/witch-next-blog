...
하지만 이후 `while(low<high)`혹은 `while(low!=high)` 등 여러 다른 방식으로 이분 탐색을 구현하시는 분들도 많다는 걸 알게 되었습니다. 이외에도 몇 가지 다른 구현 방식들이 더 있으며 또한 다른 응용들도 가능하다는 것도 알게 되었습니다.
...
물론 순차 탐색 같은 경우에도 탐색구간을 하나씩 줄여 나간다는 점에서는 이 말을 만족시킬 수 있겠으나 시간복잡도를 로그 단위로 줄여나갈 수 있을 정도로 구간을 줄이는 것이 이분 탐색의 주 목적이겠습니다.

그럼 우리는 다음과 같은 문제를 다룬다고 생각할 수 있습니다. 어떤 구간 내의 자연수에 대하여 특정 동작이 가능한지를 판단하는 결정 문제가 있다고 했을 때 해당 문제의 결과가 구간 내에서 딱 한번만 바뀐다고 합시다. 그리고 결정 문제의 결과가 구간의 하한에서는 true, 상한에서는 false라고 하면 구간에 대한 결정 문제의 결과는 `[T, T, T, ..., T, T, F, F, ..., F]`와 같은 식으로 나타날 것입니다.
...
그리고 이는 각각의 파라메트릭 서치 문제들에서 비슷하게 주어지지만 몇 가지 자잘한 부분이 바뀔 수 있습니다. 가령 구간에 대한 결정 문제의 결과가 `[F, F, ..., F, T, ..., T]`가 된다거나 구간이 자연수가 아니라 실수로 이루어져 있다든가 하는 것입니다. 하지만 이 글을 읽을 정도의 분들이라면 그 정도의 확장은 가능하리라고 생각하고, 모든 경우를 다루는 것보다는 문제는 통일하고 거기에 대한 다양한 접근을 보이는 것이 더 중요하다고 생각하여 위와 같은 문제만을 글에서 다룰 것입니다.
