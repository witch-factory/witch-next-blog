...
하지만 이후 `while(low<high)`혹은 `while(low!=high)` 등 여러 다른 방식으로 이분 탐색을 구현하시는 분들도 많다는 걸 알게 되었습니다. 이외에도 몇 가지 다른 구현 방식들이 더 있으며 또한 이러한 접근 방식 자체를 다르게 응용할 수 있다는 것도 배웠습니다.

이런 것들을 알고, 또 거기서 비롯된 이분탐색에 대한 좀더 깊은 이해를 얻기까지는 꽤 시간이 걸렸습니다. 아직 한참 부족하기는 하지만 그 흔적을 여기 적습니다.

# 1. 문제의 정의

이분 탐색의 본질을 좀더 생각해 봅시다. 이분 탐색은 결국 어떤 구간 내에서 어떤 조건의 값을 찾을 때 대소비교 등 어떤 결정 문제의 결과에 따라서 특정 구간을 탐색 대상 구간에서 제외해 나가는 것이 본질입니다.

물론 순차 탐색 같은 경우에도 탐색구간을 하나씩 줄여 나간다는 점에서는 이 말을 만족시킬 수 있겠으나 탐색의 시간복잡도가 로그 단위로 나올 정도의 속도로 구간을 줄이는 것이 이분 탐색의 주 목적이겠습니다. 구간을 반씩 줄이는 것입니다.

그럼 우리는 다음과 같은 문제를 다룬다고 생각할 수 있습니다. 어떤 구간 내의 자연수에 대하여 특정 조건이 가능한지를 판단하는 결정 문제가 있다고 했을 때 해당 문제의 결과가 구간 내에서 딱 한번만 바뀐다고 합시다. 그리고 결정 문제의 결과가 구간의 하한에서는 true, 상한에서는 false라고 하면 구간 내 수들에 대한 결정 문제의 결과는 `[T, T, T, ..., T, T, F, F, ..., F]`와 같은 식으로 나타날 것입니다.

우리는 이러한 결과 배열(?)에서 `T`로 이루어진 구간의 최댓값을 찾고자 합니다. 즉 어떤 동작이 가능하게 되는 구간의 최댓값을 구하는 것입니다. [BOJ 1654번 랜선 자르기](https://www.acmicpc.net/problem/1654)와 같은 경우라고 생각하시면 되겠습니다.

이분 탐색을 이용하면 특정 값에 대한 결정 문제 결과가 T인지 F인지에 따라서 해당 값보다 크거나 작은 구간을 탐색 구간에서 제외할 수 있고 이를 통해 시간 복잡도를 `logN`으로 할 수 있습니다.

## 1.1. 사족

결정 문제는 특정 구간 안에 특정 수가 k개 이상 있는지, 주어진 수가 해당 구간을 정렬했을 때 k번째 이상에 오는 수인지 등등 다양하게 존재할 수 있으며 흔히 `check()`함수로 나타납니다.

그리고 이는 각각의 파라메트릭 서치 문제들에서 비슷하게 주어지지만 몇 가지 자잘한 부분이 바뀔 수 있습니다. 가령 구간에 대한 결정 문제의 결과가 `[F, F, ..., F, T, ..., T]`가 된다거나 구간이 자연수가 아니라 실수로 이루어져 있다든가 하는 것입니다.

하지만 이 글을 읽을 정도의 분들이라면 그 정도의 확장은 가능하리라고 생각하고, 모든 경우를 다루는 것보다는 문제는 통일하고 거기에 대한 다양한 접근을 보이는 것이 더 중요하다고 생각하여 위와 같은 문제만을 글에서 다룰 것입니다.
...
이분 탐색과 관련된 몇 가지 추가적인 알고리즘들을 소개합니다. 저도 아주 기본적인 이해 정도만 하고 있는 부분이기도 하고 이분 탐색 자체를 깊이 이해하는 목적과는 멀리 떨어져 있다고 생각되어 직관에 도움될 만하다고 생각하는 정말 간단한 컨셉의 소개만 합니다.
