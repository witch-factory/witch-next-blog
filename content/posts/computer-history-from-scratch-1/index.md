...
하지만 그런 모든 이야기들을 다루는 건 불가능하다. 애써 시도한다고 해도 오히려 더 흥미를 잃게 만들 뿐이라고 생각한다. 위키피디아의 [Timeline of computing](https://en.wikipedia.org/wiki/Timeline_of_computing) 문서를 보면 수많은 사람들과 사건들이 나열되어 있다. 하지만 그 긴 목록을 반복하는 것이 과연 어떤 의미를 줄 수 있을까? 나의 목적은 그런 사실들의 나열이 아니다. 그 사실들이 어떻게 연결되어 있고 어떻게 인류가 컴퓨터라는 물건에 도달했고 발전시켰는지를 파헤치는 것이다.
...
앞선 두 이야기는 모두 맞다. 컴퓨터는 마술 같기도 하고, 믿을 수 없을 만큼 단순한 스위치 덩어리이기도 하다. 문제는 이 두 설명 사이에 느껴지는 커다란 간극이다. true와 false 두 가지의 논리만으로 이루어진 기계가 대체 어떻게 우리의 명령을 이해하고 실행하고 심지어 결과를 시각적으로 보여줄 수 있단 말인가?
...
그럼 어디서부터 시작할까? 방 몇 개를 통째로 채우던 최초의 컴퓨터라도 만들기까지의 흐름은 먼저 세 가지 정도로 나눌 수 있다.

- 추론 가능한 논리 체계: 논리를 통해 새로운 사실을 이끌어낼 수 있게 하는 형식과 규칙
- 논리 체계의 정립: 모든 사실을 포함하며 모순이 없는 체계를 세우고자 한 시도와 그 좌절
- 기계 장치 발전: 이론을 현실로 옮겨낼 수 있는 기초가 된 기계 장치들의 발전

그리고 이후에 이런 발전의 흐름들이 결합되는 계기가 생기고 컴퓨터가 탄생한다. 이 또한 이후에 풀어낼 예정이다. 하지만 다시 돌아가서, 가장 첫 실마리가 될 만한 걸 꼽는다면 역시 추론일 것이다. 우리가 알고 있는 사실을 바탕으로 일련의 규칙들을 통해 새로운 사실들을 만들어내고자 하는 욕심. 이 욕심에서 이야기의 첫걸음을 떼보자.
...
이 과정에서 조지 불이 등장한다. 명제들을 제대로 다루기 위해서는 이를 수학적인 형식으로 표현할 수 있어야 한다. 이렇게 명제들을 표현하고 다룰 수 있는 체계를 처음으로 제대로 세운 사람이 바로 조지 불이다. 라이프니츠가 꿈꾼 가능성을 어느 정도 이루어낸 인물이라고 할 수 있다.
...
아주 간단한 전제들이므로 논리를 조금 갖춘 사람이라면 여기서 "동생이 케이크를 먹었다"는 결론을 쉽게 이끌어낼 수 있다. 하지만 아리스토텔레스의 삼단 논법 형식으로는 이 전제들을 표현할 수 없다. 하지만 불이 세운 논리 체계에서는 가능하다. 다음과 같이 위 전제들의 구성 요소가 되는 명제를 기호로 표현할 수 있다.
