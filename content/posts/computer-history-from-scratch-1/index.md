...
1854년 불은 "생각의 법칙에 대한 탐구(An Investigation of the Laws of Thought)"라는 책을 출간한다. 이 책은 불이 세운 기호 체계와 그 체계에서의 연산들을 정리한 것이며 오늘날 불 대수로 알려진 분야의 시작이 된다.

불이 그 책을 통해 세운 가장 큰 업적은 제한된 추론 형식만을 다루던 아리스토텔레스의 체계에서 한 발 더 나아갔다는 점이다. 불은 기호를 사용해 집단을 표현하고 기호를 조작하는 방식을 세웠고 아리스토텔레스의 삼단 논법으로 포괄할 수 없는 논리들까지도 이를 통해 다룰 수 있음을 보였다. 즉 논리적 관계를 수학적으로 잘 다룰 수 있는 방법을 처음으로 제시했다.
...
그럼 자연스럽게 우리는 $B = 1$이라는 결론을 이끌어낼 수 있다. 즉 케이크를 먹은 건 동생이다. 

## 불의 꿈과 컴퓨터

불은 논리들을 기호로 표현하고 이를 풀어내는 방법을 제시했다. 또한 이렇게 논리를 조합할 수 있는 방식을 3가지만으로 한정했다. 그 3가지가 바로 오늘날의 논리합(OR), 논리곱(AND), 부정(NOT)이다. 불은 이 세 가지 연산을 통해 모든 논리적 관계를 표현할 수 있다고 주장했다.

언뜻 보면 불완전해 보이고 실제로 불완전한 부분이 있었다. 하지만 엄청나게 많아 보이는 논리의 조합 가능성을 3가지 방식만으로 표현할 수 있다는 건 대단한 발상이었다. 모든 논리 체계를 포함할 수는 없었지만, 적어도 현대의 컴퓨터는 이 3가지에 의해 돌아가고 있기에[^7].

다만 아직 실제 컴퓨터에 도달하지는 못했다. 컴퓨터에 대한 이론적 발상까지는 불의 체계에서 빈 부분을 채우려는 노력 그리고 그런 체계가 본질적으로 깰 수 없는 한계를 품고 있다는 사실을 깨닫는 일이 필요했다.

그럼에도 불이 제시한 이 새로운 형식은 분명 이후에 나올 논리 체계를 준비하는 기초가 되었다. 그럼 다음 글에서는 이러한 체계의 공백을 채우기 위해 노력했던 사람들에 대해 이어서 알아보자.
...
[^7]: 실제 컴퓨터에서는 NAND 회로만 있어도 된다. 하지만 그건 NAND 회로를 조합해서 AND, OR, NOT을 만들 수 있기 때문이다.