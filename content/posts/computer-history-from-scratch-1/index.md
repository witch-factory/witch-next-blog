---
title: 컴퓨터 연대기 1. 추론의 기초를 세운 사람들
...
하지만 그런 모든 이야기들을 다루는 건 불가능하다. 애써 해본다고 해도 그편이 더 재미없는 글이 될 거라 생각한다. 위키피디아의 [Timeline of computing](https://en.wikipedia.org/wiki/Timeline_of_computing) 문서를 보면 수많은 사람들과 사건들이 나열되어 있다. 하지만 그게 대체 무슨 의미인가? 이 글의 목적은 그런 사실들의 나열이 아니라 그 사실들이 어떻게 연결되어 있고 어떻게 인류가 컴퓨터라는 물건에 도달했고 발전시켰는지를 파헤치는 것이다.

그러니 우리에게 컴퓨터란 어떤 의미인지부터 생각해 보자. 고작 숫자, 인쇄술, 계산 따위의 확장일 뿐인가? 역사의 어느 한때에는 많은 사람들에게 컴퓨터가 그런 존재였다.

![컴퓨터 사진](./computer.webp)

하지만 지금 우리에게도 그런가? 이 사진 속에 보이는 컴퓨터가 단순한 계산기로 보이는 사람은 많지 않을 거라고 본다. 우리는 키보드 입력이나 마우스를 통한 GUI 조작 등으로 컴퓨터에게 우리가 원하는 걸 명령하고 컴퓨터는 그걸 이해한 후 거기에 맞는 동작을 마술처럼 실행해 결과를 화면에 보여준다. 뭐든 할 수 있는 도구 같다!
...
그럼 어디서부터 시작할까? 현대 컴퓨터의 기초가 되는 튜링 기계라는 개념이 나오기 전에 있었던 발전 흐름들은 세 가지 정도로 정리할 수 있다.

첫째는 논리를 통해 새로운 사실들을 이끌어내는 추론의 규칙을 세우고 그걸 일종의 계산으로 대체해 자동으로 수행하고자 한 것이다. 둘째는 논리 체계가 더 많은 사실을 포괄할 수 있도록 하고 또한 체계를 모순 없이 견고하게 세우고자 한 시도들이다. 셋째는 무언가를 자동으로 수행하려는 걸 도왔던 여러 분야에서의 기계 장치 발전이다.

그 중 가장 첫 실마리가 될 만한 건 추론일 것이다. 현재 우리가 알고 있는 사실들을 체계화하고 여기서 일련의 규칙들을 통해 새로운 사실들을 만들어내고자 하는 욕심. 이 욕심에서 이야기의 첫걸음을 떼보자.
...
현재 알고 있는 사실들로부터 새로운 판단을 이끌어낼 수 있다는 생각은 철학사의 큰 전환점들 중 하나였다. 그 전환점에는 철학에 관심이 없는 사람이라도 한번쯤은 들어보았을 고대 그리스의 철학자, 아리스토텔레스가 있다.

아리스토텔레스는 오늘날 삼단논법이라고 부르는 다음과 같은 형식의 추론을 연구했다.
...
하지만 이런 형식의 추론은 어떤 명제가 맞는지를 따지는 문제가 아니었다. 기존의 판단들을 나열하는 걸 넘어 사고의 법칙을 통해 필연적으로 따라나오는 새로운 무언가를 알아낼 수 있는 방법을 만들어냈다는 게 핵심이었다.

아리스토텔레스는 다음 네 가지 종류 중 하나의 형식을 취하는 전제들을 이용해 결론을 도출하는 규칙을 정리했다[^2].
...
이건 "논리를 통해 새로운 사실을 만들어내는 규칙"이라는 아이디어의 시작이었다. 이런 논리 체계가 더 많은 것을 포괄하도록 하기 위해 다듬고 그 체계에서 새로운 사실을 이끌어내고자 하는 시도가 이어지는 게 컴퓨터의 이론적 배경을 이룬다.

이후 전개될 여러 이야기들, 예컨대 라이프니츠와 같은 이들의 시도 역시 아리스토텔레스의 이 논리학을 바탕으로 시작한다. 하지만 그 이야기를 시작하기 앞서 그 이전에 있었던 시도들, 이 논리를 좀더 정교하게 다듬고 새로운 사실을 더 잘 이끌어내보려는 몇 가지 시도들이 있었다.

## 이후의 발전 시도

크리시포스를 비롯한 기원전의 스토아학파 철학자들은 이 논리학에 '만약 ...이면 ...이다'와 같은 조건문을 비롯한 새로운 연결사들을 도입했다. 이렇게 논리 구조를 확장해 오늘날 명제 논리로 알려진 것과 비슷한 논의를 이어갔다.

중세의 스콜라 철학자들은 "그만큼(in as much as)", "제외하고(except)"와 같은 표현들의 논리적 기능을 탐구했다. 스콜라 철학자들은 주로 이런 논리를 신에 대한 철학적 논증에 활용했다.

그런 철학자들 중 흥미로운 인물로 13세기의 철학자이자 시인 라몬 류이(Ramon Llull)가 있다. 그는 선함, 위대함, 영원 등 18가지 근본적인 개념을 조합하여 새로운 질문과 사실들을 만들어내는 방법을 제시했다. 이러한 추론을 돕는 Lullian Circle이라는 도구를 직접 만들기도 했다.

라몬 류이의 목표가 논리학의 발전은 아니었다. 그는 이런 추론을 통해 누구나 인정할 수 있는 신에 대한 완벽한 논리를 만들고자 했다. 그런 논리로 무슬림들을 설득해 기독교로 개종시키는 게 그의 목표였다. 하지만 그의 의도가 무엇이었건 그가 논리적 추론을 기계적으로 수행하려는 시도를 한 초기의 인물임은 분명하다.[^3]
...
그때부터 라이프니츠는 평생 이 마법같은 꿈을 꾸면서 살았다. 논리적인 추론을 기호의 계산으로 대체하고 이러한 계산을 수행하는 기계를 만드는 것. 이 꿈의 일부로 1673년 그는 톱니가 달린 원통들을 이용해 사칙연산을 할 수 있는 기계식 계산기를 만들었다. 여기 사용된 기계식 장치는 훗날 '라이프니츠 휠'이라고 불린다. 이전에도 덧셈과 뺄셈을 하는 계산기는 있었으나 곱셈과 나눗셈까지 가능한 계산기를 만든 건 라이프니츠가 최초였다.
...
그는 또한 이 기호 논리를 다루는 규칙들도 만들었다. 여기에는 이후 여러 학자들이 독립적으로 발견한 불 대수 속 몇몇 법칙들도 포함되어 있다. 라이프니츠는 논리항의 결합을 표현하기 위해 $\oplus$를 사용했는데 이를 차용하면 $A \oplus B = B \oplus A$, $A \oplus A = A$로 표현할 수 있는 규칙들을 제안했다.
...
우리는 $\int$ 기호를 그 자체로 적분의 의미로 받아들이고 적분을 안다면 그 기호가 포함된 식을 자연스럽게 계산해낼 수 있다. 그에게는 세상 모든 것이 이러한 보편적 의미를 지닌 기호의 계산으로 설명해낼 수 있는 걸로 보였다. 또한 이러한 계산을 언젠가는 기계가 대신할 수 있을 거라고 믿었다.

13세기의 라몬 류이가 기본적인 개념들을 통해 신의 논증을 만들어내려고 했듯 라이프니츠는 세상 모든 것을 기호로 표현하고 앞서 언급한 법칙들을 통해 추론하고자 했다.

라이프니츠의 이러한 논리학적 사유는 그가 살아 있는 동안에는 대부분 알려지지 않았고 19세기 말에 이르러서야 발견되었다. 따라서 당대나 그 직후의 논리학자들에게 라이프니츠가 끼친 영향은 크지 않다.
...
그럼 라이프니츠의 다음 걸음, 조지 불(George Boole)부터 이어가보자.
...
이후 불은 고향에서 학교를 운영하면서 동료 수학자들과 교류하던 중 이 영감을 더 발전시킨다. 그는 "어떤 강아지는 검다"와 같은 문장에 등장하는 "강아지"이나 "검은 것들"와 같이 종류를 나타내는 단어가 논리에서 중요하다는 걸 깨달았다. 그리고 이러한 종류나 집단을 표현하기 위해 영문 기호를 사용하기로 했다.

예를 들어 "강아지"를 나타내기 위해 $x$를, "검은 것"을 나타내기 위해 $y$를 사용하는 식이다. 또한 오늘날의 교집합과 같은 연산을 표현했다. $x$와 $y$가 둘 다 집단을 가리키는 기호라면 $xy$를 써서 두 가지에 모두 포함되는 것을 나타냈다. 앞서 강아지와 검은 것을 표현하는 기호를 생각하면 $xy$는 "검은 강아지"다.

그렇다면 $xx$는 무엇인가? 앞의 예시처럼 $x$가 "강아지"라면 $xx$는 "강아지이면서 강아지" 즉 "강아지"와 같다. 이 $xx = x$라는 규칙은 불의 전체 논리 체계의 기초가 된다. 또한 그는 오늘날 합집합과 차집합으로 알려진 연산을 표현하기 위해 각각 $x+y$와 $x-y$를 사용했다.

또한 불은 오늘날 컴퓨터가 이진법으로 모든 걸 나타내는 것과 비슷한 아이디어를 떠올렸다. 논리의 대수학은 0과 1로만 구성된 수학 체계와 같다는 것이다. 이는 어찌 보면 직관적인 흐름이었다. 일반적인 수학에서 언제 $xx=x$가 성립하는지 생각해 보면 $x$가 0이나 1일 경우밖에 없기 때문이다.

## 불이 세운 기호 체계

불의 가장 큰 업적은 제한된 추론 형식만을 다루던 아리스토텔레스의 체계에서 한 발 더 나아갔다는 점이다. 앞서 보았던 아리스토텔레스의 삼단 논법은 제한된 형식의 전제들에서 역시 형식에 맞춘 결론을 이끌어내는 방식이었다. 전제와 결론은 네 가지 형식 중 하나를 지켜야 했다.
...
그럼 우리는 다음과 같은 명제를 기호로 표현할 수 있다.

- $A$: 케이크를 먹은 건 엄마다.
- $B$: 케이크를 먹은 건 동생이다.

$0$을 거짓, $1$을 참으로 표현하고 불의 논리합, 논리곱 기호를 사용하면 위 전제들을 다음과 같이 표현할 수 있다.

- $A + B = 1$
- $A = 0$

그럼 자연스럽게 우리는 $B = 1$이라는 결론을 이끌어낼 수 있다. 즉 케이크를 먹은 건 동생이다. 불은 논리들을 기호로 표현하고 이를 풀어내는 방법을 제시했다. 이는 이후 컴퓨터의 기초가 되는 논리 연산을 기호로 표현하는 데 큰 영향을 미쳤다.

하지만 불이 세운 체계에는 아직 완전하지 않은 부분들이 있었다. 실제로 컴퓨터에 대한 발상에 도달하기까지는 이러한 빈 부분을 채우려는 노력과 이런 식의 체계가 본질적으로 깰 수 없는 한계를 품고 있다는 사실을 깨닫는 일이 필요했다. 이런 공백을 채웠던 사람들에 대해 다음 글에서 알아보자.
