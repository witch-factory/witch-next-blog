...
컴퓨터가 어떻게 나왔는지 따지려면 역사의 거의 모든 지점에서 이야기를 시작할 수 있다. 컴퓨터와 연관지으려면 그렇게 할 수 있는 주제들이 얼마든지 있기 때문이다. 수의 발명, 인쇄술, 문자, 숫자 0, 천문학의 계산, 주판이나 더 거슬러 올라간다면 동물의 뼛조각과 같은 계산 보조 도구들 등등.

하지만 그런 모든 이야기들을 다루는 건 불가능하다. 애써 시도한다고 해도 오히려 더 흥미를 잃게 만들 뿐이라고 생각한다. 위키피디아의 [Timeline of computing](https://en.wikipedia.org/wiki/Timeline_of_computing) 문서를 보면 수많은 사람들과 사건들이 나열되어 있다. 하지만 그 긴 목록이 과연 어떤 의미를 줄 수 있을까? 나의 목적은 그런 사실들의 나열이 아니다. 그 사실들이 어떻게 연결되어 있고 어떻게 인류가 컴퓨터라는 물건에 도달했고 발전시켰는지를 파헤치는 것이다.

그러니 우리에게 컴퓨터란 어떤 의미인지부터 생각해 보자. 고작 커다란 계산기나 정보 저장소일 뿐인가? 역사의 어느 한때에는 많은 사람들에게 컴퓨터가 정말로 그렇게 보였다.

하지만 지금 우리에게도 그런가? 지금 이 글을 읽고 있는 기기가 단순한 계산기로 보이는 사람은 많지 않을 거라고 본다. 우리는 키보드 입력이나 마우스나 터치를 통한 GUI 조작 등으로 컴퓨터에게 우리가 원하는 걸 명령하고 컴퓨터는 그걸 이해한 후 거기에 맞는 동작을 마술처럼 실행해 결과를 화면에 보여준다. 뭐든 할 수 있는 도구 같다!
...
앞선 두 이야기는 모두 맞다. 컴퓨터는 마술 같기도 하고, 믿을 수 없을 만큼 단순한 스위치 덩어리이기도 하다. 문제는 이 두 설명 사이에 느껴지는 커다란 간극이다. true와 false만으로 이루어진 기계가 대체 어떻게 우리의 명령을 이해하고 실행하고 심지어 결과를 시각적으로 보여줄 수 있단 말인가?

나는 수의 기원이나 태엽 장치처럼 우리에게 와닿지 않는 이야기를 하지 않겠다. 대신 이 두 이야기 간에 느껴지는 간극을 메우기 위해 노력하려고 한다. 논리를 기반으로 무언가를 척척 해내는 기계를 떠올리기까지 있었던 발전들을 따라가겠다. 어떤 빛나는 발상들을 통해 사람들이 컴퓨터라는 개념에 도달했으며 그걸 현실에서 사람들에게 와닿게 하기 위해 어떤 새로운 것들이 만들어졌는지 살펴보겠다. 논리가 어떻게 우리에게 마술같은 존재로 변해서 도달했는지 이해하기 위해서.

그럼 어디서부터 시작할까? 현대 컴퓨터의 기초가 되는 튜링 기계라는 개념이 나오기 전에 있었던 발전 흐름들은 세 가지 정도로 정리할 수 있다.

- 추론: 논리를 통해 새로운 사실을 이끌어내는 규칙을 세우고 그 과정을 계산으로 대체해 자동화하는 상상까지
- 논리 체계: 논리 체계가 더 많은 사실을 포괄할 수 있도록 하고 또한 체계를 모순 없이 견고하게 세우고자 한 시도들
- 기계 장치: 여러 기술적 도전과 기계들의 발전

그 중 가장 첫 실마리가 될 만한 건 역시 추론일 것이다. 우리가 알고 있는 사실을 바탕으로 일련의 규칙들을 통해 새로운 사실들을 만들어내고자 하는 욕심. 이 욕심에서 이야기의 첫걸음을 떼보자.
...
이 과정에서 조지 불(George Boole)이 등장한다. 명제들을 제대로 다루기 위해서는 이를 수학적인 형식으로 표현할 수 있어야 한다. 이렇게 명제들을 표현하고 다룰 수 있는 체계를 처음으로 제대로 세운 사람이 바로 조지 불이다. 라이프니츠가 꿈꾼 가능성을 어느 정도 이루어낸 인물이라고 할 수 있다.
...
그럼 다음 글에서는 이러한 체계의 공백을 채우고자 했던 사람들에 대해 알아보자.
