...
DNS를 사용하는 클라이언트는 아마도 컴퓨터일 테고 인터넷에 연결되어 있다. 그러면 ISP에 가입되어 있을 텐데 이 ISP가 로컬 DNS 서버(디폴트 DNS 서버라고도 한다)의 IP 주소를 제공한다. 이건 정적으로 설정되어 있거나 DHCP를 통해 동적으로 할당될 수 있다. 로컬 DNS 서버가 없는 경우에는 ISP의 DNS 서버 IP를 제공한다. 아무튼 클라이언트는 ISP를 통해 DNS 서버의 IP 주소를 알고 있다는 게 중요하다.
