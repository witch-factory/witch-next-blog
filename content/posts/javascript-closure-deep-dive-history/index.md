...
Lisp는 초기에 S-표현식과 M-표현식을 사용했다. S-표현식은 `X`, `Y`와 같은 atom과 atom들을 괄호와 점으로 연결하는 걸로 이루어졌다. 다음과 같은 형식이다.
...
M-표현식은 포트란과 비슷하게 보이는 형태로 Lisp 코드를 짤 수 있게 해주는 일종의 문법적 설탕이었다. M-표현식은 대괄호를 이용해 표기하도록 설계되었다. 그런데 매카시의 팀이 MIT에서 사용하던 천공 카드 입력기에는 대괄호 키가 없었다. 또한 편리함을 위해 M-표현식을 도입한 의도와 달리 대부분의 사람들이 S-표현식만을 사용했다. 이러한 이유들이 반영되어 이후에는 위에서 보는 것과 같은 S-표현식만이 Lisp의 표준이 되었다.
