...
하지만 내가 직접 하나하나 글을 번역하기는 현실적으로 힘들기 때문에 ChatGPT에게 맡겨 보기로 했다. 요즘 GPT API가 발전하면서 번역 품질도 높아졌다고 하니, 이를 활용하면 번역 작업을 빠르게 처리할 수 있을 것이다.

# 1. ChatGPT API 사용하기

[OpenAI 플랫폼의 Developer quickstart 문서](https://platform.openai.com/docs/quickstart)를 보면 쉽게 시작할 수 있다. 이외에도 OpenAI API를 쓰는 법에 대한 좋은 글들이 인터넷에 많기에 참고하였다.
...
아무튼 API 키를 발급받았다면 API를 실행해보자. 나는 pnpm을 사용하므로 JS/TS OpenAI SDK를 설치하자. 나는 번역용 스크립트를 수동으로 실행해서 번역을 하려고 하므로 개발 의존성에만 설치하면 된다.
...
const OPENAI_API_KEY = '여기에 API 키를 넣기';
...
실제로 번역 작업을 배치로 묶어 처리하기 위해 작성했던 함수는 다음과 같다.
...
번역 작업들을 10개씩의 배치로 묶어서 배치마다 10초의 간격을 두고 처리하도록 한 코드는 다음과 같다.
...
참고로 이렇게 210개 정도의 글을 번역하는 데에 0.5달러 정도가 들었다. 글당 대략 15000토큰 정도 소모한 꼴이다. 한국어에 비해 영어로 쓰인 글이 훨씬 많은 사람들에게 읽힐 수 있을 것을 생각하면 이 정도 비용은 저렴하다고 본다.
...
인공지능 번역은 어떤 언어로든 가능하기 때문에 일본어나 중국어 등 다른 언어로도 할 수 있을 것이다. 하지만 일단 영어에 초점을 맞춰서, 한국어 페이지와 영어 페이지를 만들어보기로 했다.

먼저 `app` 폴더의 최상단에 `[lang]` 폴더로 dynamic route를 만들었다. 그리고 기존의 페이지들을 모두 `[lang]` 폴더 안으로 옮겼다. `[lang]` 폴더 안에 있는 모든 페이지와 레이아웃 파일들의 컴포넌트는 props로 `lang`이라는 언어 코드를 받게 될 것이다.
...
현재 이 블로그에서 동작하고 있는 코드 거의 그대로이다. 여러 함수와 컴포넌트에서 `lang`을 props 혹은 인수로 받아 동작하는 것을 볼 수 있다. 그리고 섹션의 제목 등에 `content` 객체를 사용하여 `lang` 인수에 따라 다른 내용을 보여주도록 했다.

많은 다국어 관련 라이브러리에서는 `t` 함수를 통해 이런 언어별 콘텐츠 렌더링을 지원하지만 언어별 콘텐츠가 많지 않기 때문에 일단 이렇게 직접 구현하였다.
...
정석적으로는 HTTP의 콘텐츠 협상을 통해서 사용자의 언어 설정을 확인하고 그에 맞는 언어로 컨텐츠를 제공해야 한다. 하지만 이러한 다국어 처리는 고려해야 할 게 상당히 많다. 따라서 단순히 영어로도 볼 수 있는 블로그를 만들고자 했던 원래 목적을 넘어서는 일이다.

그러니 일단 간단하게 기본은 한국어로 설정하고, 사용자가 직접 언어를 선택할 수 있도록 하기로 했다. 진정한 i18n 처리에 대해서는 기회가 되면 다룰 것이다.

어쨌든 기본 경로로 접속했을 때는 무조건 한국어 페이지를 보여주도록 결정했다. 그러면 언어 정보가 없는 URL로 접속했을 경우 기본 로케일(한국어) 경로로 rewrite해주면 된다. 이 작업은 Next.js 미들웨어를 이용해서 해주었다. 이 미들웨어는 `src/middleware.ts`에 작성했다.
...
영어를 사용하는 사람을 위해 사용자가 직접 언어를 변경할 수 있는 UI를 만들어보자. 이를 위해서는 언어 변경 버튼을 만들고, 버튼을 누르면 해당 언어로 이동하도록 해야 한다. Next.js에서는 서버 컴포넌트에서 현재 경로를 알 수 있는 기능을 지원하지 않는다. 따라서 이는 클라이언트 컴포넌트여야 한다.

앞서서 한국어 페이지의 경우 경로에 언어(`/ko`, `/en`과 같은)가 포함되어 있지 않으므로 이 경우를 고려하여 언어 변경 버튼을 만들었다. 클라이언트 컴포넌트이므로 `usePathname` 훅을 사용해서 현재 경로를 알아내고 `useRouter` 훅을 사용해서 다른 언어로 이동하는 함수를 만들었다. 또한 기존 언어를 `lang` 인수로 받아서, 기존 언어와 새롭게 선택한 언어가 같은 경우에는 무시하도록 했다.
...
이 안내문 컴포넌트는 영어 글 페이지의 목차 바로 아래에 삽입하도록 했다. 이외에도 수많은 기존 함수들을 언어 기반으로 동작하도록 변경하는 노가다성 작업이 있었지만 하나하나 다룰 필요는 없을 듯 하다. 필요한 경우 [내 블로그의 레포지토리](https://github.com/witch-factory/witch-next-blog) 코드를 참고할 수 있다.
...
하지만 다국어로 가면 문제가 좀 생긴다. 물론 지원하는 언어를 하나씩 추가할 때마다 이미지의 복사본을 하나씩 더 만들도록 할 수도 있다. 하지만 이미지가 한두개도 아니고 이건 비효율적이기 때문에, 이미지는 `content/posts` 디렉토리에만 원본으로 존재하고 번역된 파일들에서는 그 원본을 복사해서 만든 `public` 디렉토리에 있는 이미지를 사용하는 게 효율적이다.
...
따라서 `<Image>` 컴포넌트의 unoptimized 옵션을 이용하여 썸네일 주소가 이걸로 시작하면 이미지 소스 최적화를 끄도록 했다. 영어 URL에 대해서도 적용해 주는 걸 잊지 말자.
...
가끔 완전히 이상한 번역을 생성하는 등의 가능성을 매번 감수해야 하는 것도 문제다. 제목에 들어가서는 안되는 특수문자(`:`같은 것)가 들어가기도 하고 번역이 제대로 되지 않는 경우도 있기 때문이다. 프롬프트를 여러 번 변경해 봤지만 다 잘 되지 않았다.

이런 점을 고려할 때 효율적으로 API 요청을 사용하고 또한 정확하게 형식을 지켜 번역하도록 하기 위해서는 내가 어느 정도의 제어를 해야 한다는 결론을 내렸다.

따라서 글의 변경이 감지되면 어떤 글이 변경되었는지 나에게 메일을 보내도록 할 것이다. 내가 내용을 보고 새로 번역이 필요하다고 판단할 경우 해당 글의 기존 번역본을 삭제하고 새로 스크립트를 돌려 번역한 후 약간의 검사를 거칠 수 있도록 말이다.
...
다음 로직을 보면 추가되거나 수정된 파일인 경우(status가 `A` 아니면 `D`) 파일의 몇 줄을 읽어서 `content`로 가져온다. `readFileLines` 함수는 스트림을 이용해서 마크다운 파일의 front matter에 해당하는 앞 몇 줄을 가져오는 함수인데 필요하면 [mailer.mjs 파일](https://github.com/witch-factory/witch-next-blog/blob/main/mailer.mjs)에서 볼 수 있다.
  ...
  const emailContent = `
      ...
      .map(
        ({ status, filePath, content }) => {
          ...
          // 필요하면 https://github.com/witch-factory/witch-next-blog/blob/main/mailer.mjs 참고
        },
      )}
  `;
...
AI를 활용하는 스크립트를 작성해 번역 작업을 처리하고 번역된 파일을 효율적으로 사용하기 위해서 블로그의 여러 코드를 뜯어고치고 몇몇을 추가했다. 이 과정에서 번역 비용을 신경써 가면서 최적화해야 하기도 했다. 이미지 경로 문제와 같은 세부적인 문제들도 있었다. 번역이 필요한 글에 대해 알람을 보내는 것도 구현하였다.

모든 작업을 완벽하게 자동화하고 싶었지만 단순하게 생각하고 내가 직접 제어하는 편이 더 쉽게 풀리는 게 많았다. 굳이 모든 걸 코드로 단순화하지 않고 GitHub Actions를 이용해 내게 알림만 오게 하고 내가 직접 처리하는 게 더 간편했다. 오히려 프롬프트를 고쳐 가면서 어떻게든 해보는 과정이 시간이 훨씬 더 많이 소모되고 얻은 건 없었다.

어쨌든 이런 과정을 거쳐 이 블로그는 한국어와 영어 모두를 지원하게 되었다. "블로그를 어떻게 영어 버전으로도 잘 보여주고 유지보수할 수 있을까" 에 초점을 맞췄기에 글에서 다루지는 않았지만 영어 글들의 SEO를 위해 사이트맵과 사이트 메타데이터, 매니페스트 파일 등에도 적절한 처리를 해주었다.

다른 언어의 추가 지원을 고려하여 다국어 컨텐츠의 지원이 쉽도록 코드를 고쳤으므로 앞으로 더 많은 언어를 지원할 수도 있을 것이다. 또한 HTTP 콘텐츠 협상을 이용한 i18n의 지원이나 더 질좋은 번역을 위한 프롬프트 엔지니어링을 해봐도 좋을 것이다. 앞으로 내 블로그가 더 넓은 세계의 사람들에게 뻗어나갈 수 있도록 계속 개선해 보려고 한다.
