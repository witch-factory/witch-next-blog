...
식별자 `a`가 정의되어 있지 않으므로 호출시 에러가 발생할 것이다. 외부에 의존하는 값이 있어서 `inner` 호출의 평가 결과는 단독으로 결정될 수 없기 때문이다. 클로저는 이렇게 외부에 의존하는 값을 갖는 표현식의 평가 결과를 외부 환경과 함께 묶어서 힙에 저장함으로써 해당 표현식이 사용될 때 외부 환경에 대한 정보를 얻을 수 있게 한다.
