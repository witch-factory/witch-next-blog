---
title: JS 탐구생활 - 클로저 연대기 2. 클로저의 역사
date: "2024-10-25T00:00:00Z"
description: "클로저, 수학에서 JavaScript의 스타가 되기까지의 역사"
...
- 클로저가 진짜로 대체 무엇이고 무엇을 할 수 있는 걸까?
- 클로저는 대체 어디서 나와서 무슨 문제를 해결했고 어떻게 JavaScript까지 들어가서 이렇게 유명해졌을까?
...
클로저가 무엇인지 알았다고 하자. 그럼 **클로저는 대체 왜 이리 중요하고 많이 나오는가?** 그만큼 많이 쓰이고 응용되어서 그렇다고 간단히 대답할 수도 있겠지만, 수많은 대답과 질문을 꼬리물어볼 수 있다.

왜 JavaScript에서 클로저가 이렇게 중요하게 다루어질까? 클로저가 있는 다른 언어들도 있는데 말이다. 그 답은 클로저가 있는 다른 언어들과 달리 JavaScript는 처음부터 함수형의 영향을 받아 만들어지기도 했고 class를 비롯한 객체지향 관련 문법도 오랫동안 없었기에 객체지향 구현에서 클로저를 많이 사용했기 때문이다.

그럼 왜 JavaScript에는 클로저가 들어갔을까? 클로저가 있던 함수형 언어인 Scheme의 영향을 받았기 때문이다.

이 클로저는 왜 나왔을까? 람다 계산법[^1]을 프로그래밍 언어 상에서 구현하는 과정에서 funarg problem을 해결하려고 나왔다.

그럼 왜 람다 계산법을 프로그래밍 언어로 구현해야 했을까? 모든 기계적인 계산을 해내는 프로그래밍 언어를 만들기 위한 한 가지 방법이었기 때문이다.

그럼 왜 모든 기계적인 계산을 해내야 했을까? 그게 프로그래밍 언어고 컴퓨터니까.

클로저에서 이야기가 많이 부풀어올랐다. 그럼 이제 다시 오래전부터 시작해보자. 프로그래밍 언어가 만들어지던 시절로 돌아갔다가 다시 JavaScript 진영의 스타가 되기까지 가보는 것이다.
...
기계적인 방식만으로 모든 참인 명제를 찾아내는 게 불가능하다는 것은 알았다. 그런데 이 "기계적인 방식" 그리고 "기계적으로 계산 가능"이란 대체 무엇인가? 이를 정의하기 위해 나왔던 방식 중 하나가 람다 계산법이다. 하지만 람다 계산법이 나오기까지 이전의 정의들도 있었다.

당연하지만 불완전성 정리를 처음 증명한 괴델이 이에 대해 가장 먼저 정의했다. 괴델은 1931년 불완전성 정리를 증명하면서 부분 재귀 함수라는 것을 정의하고 "부분 재귀 함수 꼴로 정의되는 함수"를 기계적으로 계산 가능하다고 했다.
...
그 비슷한 시기, 알론조 처치가 드디어 람다 계산법을 정의한다. 원래 이는 기존 수학 체계 구성에 많이 쓰였던 집합 대신 함수를 이용해서 새로운 논리 체계를 구성하려는 시도에서 나왔지만 1930년대 초 그 체계가 일관성이 없다는 게 밝혀지면서 실패했다[^4]. 그러나 체계 구성은 실패했어도 함수에 관한 탄탄한 계산 이론이 남았는데 이를 정리해서 만든 게 바로 람다 계산법이다. 그리고 처치는 "람다 계산법으로 계산될 수 있는 것들"을 계산 가능하다고 정의했다.
...
>
> Kenneth E. Iverson, 튜링 상 기념 강연 중

클로저가 나오기까지는 아직 시간이 좀 걸린다. 이건 컴퓨터의 초기 시대에, 함수를 프로그래밍의 중심에 둔다는 생각을 아무도 하지 않았기 때문이다. 최초의 함수형 언어라고 할 수 있는 Lisp를 만든 매카시조차도 오랫동안, 프로그래밍을 단계별로 실행되는 알고리즘의 설계로 생각했다.

이는 컴퓨터가 어떻게 등장했는지에 영향을 받는다. 이는 초기 컴퓨터가 튜링 기계를 기반으로 했기 때문이다. 튜링 기계에 관해 자세히 설명하는 것은 이 글의 범위를 벗어나지만, 작동규칙표에 작동하는 단순한 기계라고 볼 수 있다. 오늘날 명령형 프로그래밍이라고 불리는 것과 비슷하다고 보면 된다.

이 튜링 기계를 통해서 모든 계산 가능한 것을 계산할 수 있고 현재 존재하는 모든 프로그램을 표현할 수 있다. 그리고 이렇게 주어진 명령대로 움직이는 튜링 기계를 전기 회로로 구현한 것이 최초의 컴퓨터였다. 1937년 클로드 섀넌(놀랍게도 당시 21세)이 전기 스위치로 튜링 기계를 만들 수 있다는 가능성을 보였고, 1948년 맨체스터 대학에서 맨체스터 마크 원이, 1952년 폰 노이만이 EDVAC을 만들었다.

물론 람다 계산법도 똑같은 능력을 가지고 있다. 만약 람다 계산법을 기반으로 한 명령을 알아듣는 기계가 최초의 컴퓨터로서 만들어졌다면 프로그래밍 언어의 역사는 많이 달랐을지도 모른다. 하지만 최초의 컴퓨터는 튜링 기계 기반이었고, 당연히 초기 프로그래밍 언어도 튜링 기계의 명령어를 기반으로 만들어졌다. 함수형 언어에서 나온 개념인 클로저가 나올 수 있는 환경이 아니었던 것이다.
...
다음 섹션은 람다 계산법을 프로그래밍 언어로 구현하기까지의 과정에 대한 내용이다. 람다 계산법의 개념들이 어느 정도 언급될 예정이므로 그전에 람다 계산법을 간단히 알아보자.
...
같은 논문에서 랜딘은 이를 기계적으로 평가하는 방법을 수학적으로 기술한 SECD Machine을 제시하고, 1966년의 다른 논문[^11]에서 이를 기반으로 이론적인 언어 모델인 ISWIM을 제안한다. SECD Machine과 ISWIM에 영감을 받아 프로그래밍 언어론 교육을 위한 언어이며 일급 객체 함수를 지원한 PAL이 나오지만 실용적인 언어의 계보를 만들지는 못했다.
