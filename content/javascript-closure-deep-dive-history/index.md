...
그런데 사실은 초기의 Lisp에서도 함수를 다른 함수의 인자로 전달할 수 있었다. QUOTE를 사용해 함수를 선언하면 표현식을 평가하지 않고 일종의 데이터 취급을 할 수 있었고 따라서 인자로 전달도 가능했다. 오늘날의 메타 프로그래밍과 비슷한 기능이었다.
...
Algol은 함수를 호출하면 생기는 스택 프레임에 함수를 호출한 위치의 참조뿐 아니라 함수를 선언한 곳에 대한 정적 참조도 추가했다. 인자로 전달된 함수가 쓰일 땐 그 함수의 스택 프레임에 정적 링크로 연결된 스코프를 따라가서 변수를 찾으면 됐다. 이는 잘 작동했다.
...
초기 Lisp의 QUOTE는 함수를 선언할 때 표현식의 평가를 생략하고 일종의 데이터로 다룰 수 있게 해주는 기능이었다. 그래서 오늘날 일반적으로 말하는 동적 스코프와는 약간 다르다. 굳이 가까운 걸 찾자면 함수를 선언할 때 `eval`로 평가할 문자열, 이를테면 `(x) => "(y) => x + y"`를 문자열 형태로 그대로 선언하는 것과 비슷하다.
...
```js
// QUOTE 키워드로 선언할 시 함수 선언부에서 표현식의 평가를 생략하고 그대로 다룰 수 있다
// 실제로 초기 Lisp에서는 인자를 데이터로만 취급하는 QUOTE, 함수 객체로 취급하는 FUNCTION 키워드가 있었다
// 함수를 평가하지 않고 그대로 다루도록 한다
QUOTE outer(x) {
  return `y => ${x} + y`;
}

function foo(quotation, y) {
  // JavaScript의 eval이 아니라 코드를 그대로 평가하는 일반적인 의미로만 생각하자
  const innerFunction = evalquote(quotation); // 전달된 QUOTE 내용을 평가
  return innerFunction(y); // y를 인자로 전달해 호출
}

/**
 * outer가 받는 인수에 3이 들어가서 inner가 "y => 3 + y"가 되고 
 * y 자리에는 2가 들어가서 결과 5를 반환하기를 기대한다
 * 하지만 y => x + y가 그대로 전달되어 inner는 (y => x + y)(y)가 되고
 * inner(2)는 (y => x + y)(2), 즉 x + 2로 평가되는데 이 스코프에 x는 없으므로 에러 발생
*/
const inner = outer(3);
const result = foo(inner, 2); // x is not defined 에러 발생
```
