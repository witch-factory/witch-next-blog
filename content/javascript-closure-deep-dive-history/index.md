...
description: "수학, 기계적인 계산, 람다 계산법, 컴퓨터, Lisp, Algol을 거치면서 생긴 표현식의 평가에 대한 문제와 해결, 이 흔적을 오늘날 클로저라 부른다."
...
"기계적인 계산"이란 대체 무엇인가? 어떤 조건을 만족해야 기계적인 방식으로 계산 가능하다고 할 수 있으며 자동으로 할 수 있으리라는 꿈을 꿀 수 있을까? 당연하지만 불완전성 정리를 처음 증명한 괴델이 이에 대해 가장 먼저 정의했다. 괴델은 1931년 불완전성 정리를 증명하면서 부분 재귀 함수라는 것을 정의하고 "부분 재귀 함수 꼴로 정의되는 함수"를 기계적인 방식으로 계산 가능하다고 했다.
...
그 비슷한 시기, 알론조 처치가 람다 계산법을 통해 계산 가능성을 정의한다. 원래 이는 기존 수학 체계 구성에 많이 쓰였던 집합 대신 함수를 이용해서 새로운 논리 체계를 구성하려는 시도였다. 처치는 이 체계가 기존 체계보다 훨씬 간단하며 러셀이 기존 체계에 제기했던 역설도 피할 수 있을 거라고 생각했다. 하지만 1930년대 초 처치 자신의 제자였던 클레이니와 로서에 의해 이 체계가 일관성이 없다는 게 밝혀지면서 실패했다[^4].

하지만 처치는 좌절하지 않았고 거기서 건질 수 있는 것을 건져냈다. 그리고 그가 맞았다. 완벽히 일관적인 논리 체계 구성은 실패했어도 함수에 관한 탄탄한 계산 이론이 남았기 때문이다. 그렇게 목표를 좀 더 줄여 수정한 체계를 만든 것이 람다 계산법이다. 처치는 "람다 계산법으로 계산될 수 있는 것들"을 계산 가능하다고 정의했다.

곧 밝혀졌지만 괴델, 튜링, 처치가 각자 내린 이 세 정의는 모두 동치이다. 셋 중 하나의 "계산 가능"정의만 구현하면 대부분의 프로그래밍 언어가 목표로 하는 "튜링 완전"하다고 할 수 있다. 달리 말하면 이 세 정의 모두 현재 존재하는 프로그램을 전부 표현할 수 있다는 것이다. 물론 이 셋 외에도 튜링 완전을 정의하는 방법이 여러 가지 있지만 이 셋이 가장 대표적이다.
...
이 튜링 기계를 통해서 모든 계산 가능한 것을 계산할 수 있고 현재 존재하는 모든 프로그램을 표현할 수 있다. 그리고 1937년 클로드 섀넌(놀랍게도 당시 21세)이 전기 스위치로 튜링 기계를 만들 수 있다는 가능성을 보였다. 그리고 이게 실현되어 1948년 맨체스터 대학에서 맨체스터 마크 원이 만들어졌고 1952년 폰 노이만이 EDVAC을 만들었다. 즉 최초의 컴퓨터들은 튜링 기계 이론을 기반으로 한 명령어를 알아듣는 기계였다. 그리고 이게 지금까지 이어지고 있다.

물론 람다 계산법도 똑같은 능력을 가지고 있다. 만약 람다 계산법을 기반으로 한 명령을 알아듣는 기계가 최초의 컴퓨터로서 만들어졌다면 프로그래밍 언어의 역사는 많이 달랐을지도 모른다. 하지만 최초의 컴퓨터는 튜링 기계 기반이었고, 당연히 초기 프로그래밍 언어도 튜링 기계의 명령어와 사고방식을 기반으로 만들어졌다. 최초의 컴퓨터가 생기던 시절은 함수형 언어에서 나온 개념인 클로저가 나올 수 있는 환경이 아니었던 것이다.
...
추상화는 앞서 말했듯 프로그래밍하며 흔히 다루는 함수와 비슷하다. 그런데 문제가 있다. 형식상 추상화가 받는 인자는 하나뿐이다! 이걸로 어떻게 인자가 여러 개인 일반적인 함수를 표현할 수 있을까?

고차 함수를 이용해서 할 수 있다[^6]. 예를 들어 2개의 인자를 받아 더하는 다음과 같은 함수를 만들고 싶다고 하자.
...
그리고 마지막으로, 이런 고차 함수에서 등장하는 bound variable과 free variable에 대해 알아보자. 간단히 설명하면 람다 계산법의 표현식 내에서 값을 찾을 수 있으면 bound variable이고 그렇지 않으면 free variable이라고 할 수 있다.

람다 계산법에서는 추상화 $\lambda x.y$ 에서 $y$ 표현식 내에서 $x$가 나타나면 bound라고 한다. 또한 이렇게 $x$와 $y$를 연관시켜주는 $\lambda x.$를 "binder"라고 한다. 예를 들어 $\lambda x.x+1$에서 $x$는 bound variable이다.
...
그리고 그 당시 인공지능 연구자였던 존 매카시가 있었다. 그는 1955년 이전부터 인공지능 연구에서 프로그래밍 언어가 얼마나 중요한지 느끼고 있었고 표현식들의 결합으로 나타낼 수 있는 수학적이고 논리적인 언어를 꿈꿨다.
...
하지만 그가 설계하던 시스템에서 필요한 재귀, 함수 인수 등을 지원하기 위해서는 완전히 새로운 언어가 필요하다는 것이 점점 명확해졌다. 마침 설립된 MIT 인공지능 프로젝트의 지원을 받아 매카시는 1958년 최초의 함수형 프로그래밍 언어 Lisp를 만든다.
...
Algol의 영향이라고 할 수는 없겠지만 이는 지금도 JavaScript 함수 객체가 생성될 때 `[[Environment]]` 내부 슬롯에 함수를 선언한 환경의 참조를 저장함으로써 비슷하게 나타난다.

하지만 그런 방식이 잘 작동한 것은 앞서 언급했듯 Algol에서는 함수를 결과로 반환하는 기능이 없었기 때문이다. Algol에서 함수가 다른 함수에 인자로 전달되더라도 인자로 전달된 함수를 선언한 스코프의 스택 프레임은 늘 콜스택에 살아 있었다. 그렇기에 이런 방식으로 문제를 해결할 수 있었다. 
...
랜딘은 이 논문에서 재귀식 등 프로그래밍에서 발생하는 다양한 구조들을 람다 계산법 형식의 표현식(lambda expression이라고 정의한다)으로 표현하는 것을 보인다. 그리고 이를 어떻게 기계적으로 평가할지를 논한다. 그러면서 표현식을 평가함에 있어 외부 환경을 고려해야 한다는 일견 당연한 사실을 이야기한다. 표현식 $X$를 평가할 때 $X$가 평가되는 환경이 $X$의 free variable에 어떤 값을 부여하는지에 따라 결과가 달라진다는 것이다.

따라서 외부 환경 $E$에 대해 표현식 $X$를 평가하는 걸 $val(E)(X)$로 표현한다. 이어서 랜딘은 이런 표현식의 평가 결과는 표현식과 그게 평가되는 환경의 묶음으로 정의해야 한다고 주장하고 그 묶음을 클로저라고 정의한다.. 외부 환경에서 평가된 이후에도 또 다른 외부 환경에서 평가될 수 있는 경우가 있기 때문이다. 고차 함수의 경우를 생각해보자.
...
let a = 1, b = 2, c = 3;
const middle = grandparent(a);
const result = middle(b)(c);
```

`result`가 최종적으로 나오기 전에 `middle`이라는 함수를 만들었다. 이건 어떤 값으로 확정될 수 있을까? 아직 자유 변수를 가지고 있기 때문에 확정될 수 없다. 그리고 `middle`의 평가 결과 또한 따져보면 `a` 식별자가 1로 바인딩된 외부 환경에 기반을 두고 있다.

따라서 전역 환경을 $E$라고 할 때 `middle`은 $E$에서 `grandparent`를 평가한 것이고 `middle`이 호출된 결과를 평가한다는 건 $E$에서 파생된 `a`가 고정됨으로써 파생된 새로운 환경 $E'$에서 `parent`를 평가한다는 것이다. 이를 일반화해 생각하면 표현식의 평가 결과는 표현식과 환경의 묶음인 클로저여야 한다.
...
또한 1960년대 후반에 MIT의 인공지능 연구실에서 박사 과정을 하고 있던 칼 휴이트도 메시지를 주고받는 것으로 구조를 형성하는 방식에 영감을 받아 지식 추론 시스템인 Planner를 구상했다. 이 Planner는 이후 다른 유명한 인공지능 언어인 Prolog를 탄생시키기도 했다.

이렇게 비슷한 뿌리에서 Smalltalk을 만든 케이와 Planner를 만든 휴이트는 1970년대 초에 서로 교류하면서 각자의 지식을 발전시켜 나간다. 그런데 Planner는 너무 방대하고 복잡했고 당시 컴퓨터 환경에서는 한계가 있는 기능도 있었다.

Planner의 이런 문제를 해결하기 위해 휴이트와 그의 제자들은 Simula와 Smalltalk의 구조에 "액터 모델"이라는 걸 만들었다. 모든 데이터를 추상화해서 액터로 보는 거였다. 모든 계산 주체와 데이터는 액터이며 관계를 맺고 있는 다른 액터들(acquaintance라고 했다)이 있다. 그리고 이런 액터들이 메시지를 주고받는 방식으로 계산을 진행한다. 그리고 이러한 메시지 통신은 비동기적으로 이루어진다.

보기만 해도 매우 어렵다. 이 글의 주요 주제도 아닌 액터 모델을 깊게 이해할 필요도 없다. 중요한 건 이게 당시 MIT의 대학원생이었던 가이 스틸과 그의 지도교수 제럴드 서스만에게도 마찬가지로 어려웠다는 사실이다.

그들은 이 액터 모델을 이해하고 싶었으나 쉽지 않았다. 그래서 오늘날에도 많은 개발자들이 라이브러리를 익히기 위해 토이 프로젝트를 해보듯이, 그들은 액터 모델을 구현하는 작은 장난감 언어를 만들어 보기로 했다. 이게 Scheme의 시작이었다.
...
스틸과 서스만은 작은 Lisp 인터프리터에 액터를 생성하는 것, 메시지를 보내는 것 2가지만 추가한 언어를 만들기로 했다. 그리고 당시 제럴드 서스만이 학교에서 Algol 60을 가르치고 있었기에 그 영향으로 렉시컬 스코프를 사용하게 되었다.

그리고 언어를 만들고 칼 휴이트와 이야기를 나누면서 스틸과 서스만은 액터 모델이 람다 계산법을 이용한 표현식을 쓰는 것과 거의 같다는 걸 깨달았다. 그저 같은 대상을 어떤 관점에서 바라보느냐의 차이일 뿐이었다. 그걸 깨닫고 나자 그들에게 남은 건 몇 가지 좋은 특성을 가진 작은 Lisp 방언이었다.

1975년 가이 스틸과 제럴드 서스만은 그때까지의 결과를 모아 Scheme을 발표한다. 당시 인공지능 언어들의 이름이 "-er"로 끝나던 관례에 따라 "Schemer"로 하려 했지만 당시 그들이 쓰던 운영체제의 파일명 길이 제한이 6자라서 "Scheme"이 이름이 되었다. 시작이야 어쨌든 이 언어는 이제 확실하게 람다 계산법을 기반으로 하고 있었다. 언어의 튜토리얼 문서 제목부터가 "Scheme: An Interpreter for Extended Lambda Calculus"였다.
...
이제 산업계에서는 사용자가 페이지와 상호작용하고 또 애플리케이션을 직접 만들 수 있도록 하기 위해 스크립트 언어를 개발하고자 했다. JavaScript의 창시자 브랜든 아이크가 이를 위해 1995년 4월 넷스케이프에 합류한다. 네트워크 작업을 위한 작은 특수목적 프로그래밍 언어를 해본 경험 덕분이었다.
...
하지만 아이크에게는 일정 압박과 몇몇 제한 이외에는 매우 많은 결정권이 있었다. 비록 Scheme을 구현할 수는 없었지만 아이크는 여전히 일급 함수 개념에 매력을 느꼈기에 JavaScript에 도입할 수 있었다. 함수는 일급 객체로 취급되었고, 람다 표현식의 구현은 시간이 없어서 연기되었지만 예정되어 있었다. 또한 몇몇 버그가 있었지만 렉시컬 스코프를 사용했다. 이런 결정들은 자연스럽게 클로저를 JavaScript에 들여왔다.
...
JavaScript는 웹에서 사용자와 상호작용하기 위해 만들어졌다. 그리고 매우 심한 일정 압박과 인원 부족 속에서 만들어졌다. 또한 "Java의 보조 언어"라는 언어의 탄생 배경상 Java와 비슷해야 했고 초보자도 쉽게 쓸 수 있어야 한다는 요구사항도 있었다.

이러한 상황 속에서 아이크 또한 여러 실수를 하기도 했다. 하지만 그는 적어도 짧은 기간 내에 많은 것을 하겠다고 욕심을 부리지 않았다. 그는 스스로 생각하는 중요한 사항을 빼면 나머지는 매우 유연하게 만들고 거의 모든 것을 수정 가능하게 했다. 그렇게 모든 걸 유연하게 만들어 놓으면 내장 라이브러리 같은 건 날림으로 만들어 두거나 없더라도 다른 개발자들이 수정해 사용하리라 생각했다. 

그리고, 열흘간의 촉박한 프로토타이핑 기간 동안 아이크가 양보할 수 없었던 '그 중요한 사항' 중 하나가 바로 Scheme에서 온 일급 객체 함수의 개념이었다. 나머지 하나는 Self에서 온 프로토타입 기반 상속이었다. 나머지는 멀티 패러다임 언어로 설계되었다고 당당히 말할 수도 없을 만큼 날림인 게 많았다.

Date 클래스는 Java의 `java.util.Date`를 대충 가져온 거였고 `==`나 `with`는 버그를 펑펑 터뜨렸다. 프로토타입도 단일 상속밖에 되지 않았고 접근 제한자, getter, setter같은 정보 은닉 기능도 없었다. 클래스, 모듈, 네임스페이스 등 객체지향 기능도 없었고 그렇다고 함수형 프로그래밍이나 비동기 관련 지원이 많은 것도 아니었다.

그런데 시간이 지나면서 JavaScript는 본의 아니게 웹의 주류 언어가 되었고 웹이, 그러니까 개발자들이 JavaScript로 해야 할 일은 점점 많아졌다. 하지만 언어 표준에 새로운 기능이 도입되기까지는 시간이 오래 걸린다. 그래서 개발자들은 언어의 기능을 확장하기 위해 클로저를 많이 사용했다.

객체를 리턴하는 함수를 만들고 함수 스코프 내에 변수를 숨겨서 정보 은닉을 구현했다. 코드를 즉시 실행 함수(IIFE)에 넣어서 네임스페이스와 모듈을 구현했다. 디자인 패턴 구현에도 많이 사용되었고, DOM 조작이나 이벤트 핸들러 등의 다양한 기능을 구현하는 데에도 사용했다. 함수의 실행과 같은 메타 정보를 추적하기도 했다. 프로토타입 상속의 부족한 점을 보완하기 위해서 만드는 보조 함수에도 클로저가 쓰였다.

이런 과정을 거치면서 클로저는 JavaScript에서 매우 중요한 개념 중 하나로 자리 잡게 되었다. 당시에 존재했던 다른 언어들에도 클로저는 존재했기에 지식 자체가 중요했다고 보기에는 어렵다. 그저 JavaScript에서 클로저가 너무, 너무 많이 쓰였다.
...
Lisp의 quote는 함수를 선언할 때 표현식의 평가를 생략하고 일종의 데이터로 다룰 수 있게 해주는 기능이다. 그래서 오늘날 일반적으로 말하는 동적 스코프와는 약간 다르다. 굳이 가까운 걸 찾자면 함수를 선언할 때 `eval`로 평가할 문자열, 이를테면 `(x) => "(y) => x + y"`를 문자열 형태로 그대로 선언하는 것과 비슷하다.

따라서 quote로 선언된 함수는 표현식의 평가를 생략한 채 그대로 쓰일 수 있었고 함수에 인자로 전달하는 것도 가능했다. 오늘날의 개념으로 가까운 걸 찾는다면 `eval`로 평가할 문자열, 이를테면 `(x) => "(y) => x + y"`를 문자열 형태로 그대로 전달하는 것과 비슷하다.

```lisp
;; quote를 사용하여 함수 코드의 평가를 생략하고 일종의 데이터처럼 그대로 다루도록 한다
;; quote를 쓰면 함수를 데이터로 취급하게 되고 function을 쓰면 함수 객체로 취급하게 된다
(defun outer (x)
  (quote (lambda (y) (+ ,x y))))

(defun foo (quotation y)
  ;; quotation 인자를 통해 전달된 문자열을 코드로 가정해서 그대로 평가 
...
하지만 quote 선언의 특성상 y => x + y가 그대로 전달되어 inner는 (y => x + y)(y)가 되고
inner(2)는 (y => x + y)(2), 즉 x + 2로 평가되는데 이 스코프에 x는 없으므로 에러 발생"
(let ((inner (outer 3)))
